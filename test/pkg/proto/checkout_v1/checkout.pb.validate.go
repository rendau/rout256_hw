...
// ValidateAll checks the field values on ListCartResponseItem with the rules
// defined in the proto definition for this message. If any rules are
// violated, the result is a list of violation errors wrapped in
// ListCartResponseItemMultiError, or nil if none found.
func (m *ListCartResponseItem) ValidateAll() error {
	return m.validate(true)
}

func (m *ListCartResponseItem) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	if len(errors) > 0 {
		return ListCartResponseItemMultiError(errors)
	}

	return nil
}

// ListCartResponseItemMultiError is an error wrapping multiple validation
// errors returned by ListCartResponseItem.ValidateAll() if the designated
// constraints aren't met.
type ListCartResponseItemMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m ListCartResponseItemMultiError) Error() string {
	var msgs []string
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m ListCartResponseItemMultiError) AllErrors() []error { return m }
